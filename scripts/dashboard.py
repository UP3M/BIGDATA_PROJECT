...
def first_reg(string1):
    # regular expression
    pattern = r"name='maxIter'.+?\): (\d+)"
    pattern1 = r"name='regParam'.+?\): (\d.\d+)"
    matches = re.search(pattern, string1)
    matches1 = re.search(pattern1, string1)
    return "regularization param:{}, maxIter:{}".format(matches1.group(1), matches.group(1))
...
q6 = pd.read_csv("output/q6.csv/part-00000-88d153c8-24eb-4e33-9d06-40e55dc77386-c000.csv")
q6.iloc[:, 1] = q6.iloc[:, 1].apply(second_reg)
q7 = pd.read_csv("output/q7.csv/part-00000-64112468-f2a8-4191-b1fe-ec0a5f4e39d4-c000.csv")
#st.table(q7)
